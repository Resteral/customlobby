...
public static class Building
{
    ...
    public static BuilderData? GetBuilder(CCSPlayerController player)
    {
        if (player == null || !player.IsValid) return null;

        if (Builders.TryGetValue(player.Slot, out var data))
        {
            return data;
        }
        ...
        {
            var newData = new BuilderData { BlockType = Blocks.Models.Data.Platform.Title };
            Builders[player.Slot] = newData;
            return newData;
        }
    ...
    public static void OnTick()
    {
        ...
        {
            Console.WriteLine($"[BlockMaker] Exception in Building.OnTick: {ex.Message}\n{ex.StackTrace}");
        }
    }
    ...
    private static void GrabBlock(CCSPlayerController player)
    {
        ...
        if (entity != null)
        {
            ...
            // Use the entity's own origin as the aim point (no TraceShape needed since GetBlockAim() found the entity)
            var endPos = entity.AbsOrigin!.ToVector_t();
