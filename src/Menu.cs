...
public partial class Menu
{
    ...
    private static readonly Dictionary<int, int> _playerMenuPages = new();

    public static IMenu Create(string title, string menuType, IMenu? prevMenu = null)
    {
        IMenu menu = CustomMenuManager.MenuByType(menuType, title, Instance);
    ...
    public static void Open(CCSPlayerController player, string title)
    {
        ...
        Menu.AddItem("Create Block", (player, option) =>
        {
            Commands.CreateBlock(player);
            Open(player, title);
        });

        Menu.AddItem("Delete Block", (player, option) =>
        {
            Commands.DeleteBlock(player);
            Open(player, title);
        });

        Menu.AddItem($"Select Block Type: {BuilderData.BlockType}", (player, option) =>
        {
            _playerMenuPages[player.Slot] = 0; // always start from first page
            TypeMenuOptions(player, menuType, Menu);
        });

        Menu.AddItem($"Select Block Color: {BuilderData.BlockColor}", (player, option) =>
        {
            ColorMenuOptions(player, menuType, Menu);
        });

        Menu.AddItem($"Select Block Size: {BuilderData.BlockSize}", (player, option) =>
        {
            ...
            Utils.PrintToChat(player, $"Selected Size: {ChatColors.White}{BuilderData.BlockSize}");
            Open(player, title);
        });

        Menu.AddItem("Move Block Menu", (player, option) =>
        {
            string[] options = { "X-", "X+", "Y-", "Y+", "Z-", "Z+" };
            PositionMenuOptions(player, menuType, Menu, options, false);
        });

        Menu.AddItem("Rotate Block Menu", (player, option) =>
        {
            string[] options = { "Reset", "X-", "X+", "Y-", "Y+", "Z-", "Z+" };
            PositionMenuOptions(player, menuType, Menu, options, true);
        });

        Menu.AddItem("More Block Settings & Actions...", (player, option) =>
        {
    ...
    static void PositionMenuOptions(CCSPlayerController player, string menuType, IMenu Parent, string[] options, bool rotate)
    {
        ...
        string title = $"{(rotate ? "Rotate" : "Move")} Block ({value} {(rotate ? "Deg" : "Units")})";

        IMenu Menu = Create(title, menuType, Parent);

        string cycleLabel = rotate 
            ? $"Cycle Rotation Step: {BuilderData.RotationValue}°" 
            : $"Cycle Nudge Step: {BuilderData.PositionValue} Units";

        Menu.AddItem(cycleLabel, (player, option) =>
        {
            if (rotate)
            {
                float[] rotations = { 15f, 30f, 45f, 90f, 180f };
                int idx = Array.IndexOf(rotations, BuilderData.RotationValue);
                BuilderData.RotationValue = rotations[(idx + 1) % rotations.Length];
            }
            else
            {
                float[] steps = { 1f, 4f, 8f, 16f, 32f, 64f, 128f };
                int idx = Array.IndexOf(steps, BuilderData.PositionValue);
                BuilderData.PositionValue = steps[(idx + 1) % steps.Length];
            }
            PositionMenuOptions(player, menuType, Parent, options, rotate);
        });

        Menu.AddItem($"Custom Units (Chat)", (player, option) =>
        {
    ...
    static void Menu_BlockSettings(CCSPlayerController player, string menuType, IMenu Parent)
    {
        ...
        IMenu Menu = Create("Block Settings & Actions", menuType, Parent);
        ...
        Menu.AddItem("Copy Aimed Block", (player, option) =>
        {
            Commands.CopyBlock(player);
            Menu_BlockSettings(player, menuType, Parent);
        });

        Menu.AddItem("Convert Aimed Block", (player, option) =>
        {
            Commands.ConvertBlock(player);
            Menu_BlockSettings(player, menuType, Parent);
        });

        Menu.AddItem("Lock/Unlock Aimed Block", (player, option) =>
        {
    ...
    static void TypeMenuOptions(CCSPlayerController player, string menuType, IMenu Parent)
    {
        ...
        IMenu Menu = Create($"Select Type ({BuilderData.BlockType})", menuType, null);

        if (!_playerMenuPages.TryGetValue(player.Slot, out int page))
        {
            page = 0;
            _playerMenuPages[player.Slot] = page;
        }

        var blockmodels = Blocks.Models.Data;
        var allBlocks = blockmodels.GetAllBlocks().ToList();
        int itemsPerPage = 7;
        int totalPages = (int)Math.Ceiling((double)allBlocks.Count / itemsPerPage);

        if (page < 0) page = 0;
        if (page >= totalPages) page = 0;

        int startIndex = page * itemsPerPage;
        var pageBlocks = allBlocks.Skip(startIndex).Take(itemsPerPage).ToList();
        ...
        // Pad with empty items to ensure we always have 7 options
        while (pageBlocks.Count < 7)
        {
            Menu.AddItem(" ", (player, option) => {
                TypeMenuOptions(player, menuType, Parent);
            });
            pageBlocks.Add(null!);
        }

        // Option 8: Go back to parent menu
        Menu.AddItem("Back to Main Menu", (player, option) =>
        {
            _playerMenuPages[player.Slot] = 0; // reset so next open starts at page 0
            Parent.Display(player);
        });

        // Option 9: Next Page
        Menu.AddItem("Next Page", (player, option) =>
        {
            _playerMenuPages[player.Slot] = (page + 1) % totalPages;
            TypeMenuOptions(player, menuType, Parent);
        });
