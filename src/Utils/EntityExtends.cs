...
public static class EntityExtends
{
    ...
    public static void CollisionRulesChanged(this CBaseEntity entity, CollisionGroup group)
    {
        ...
        if (group == CollisionGroup.COLLISION_GROUP_TRIGGER)
        {
            entity.Collision.SolidFlags = 4; // FSOLID_NOT_SOLID (4) makes player fall through
        }
        else
        {
            entity.Collision.SolidFlags = 0; // Solid (0) restores normal collision
        }
    }
}